    ...
    <section class="add-goods-page" v-loading="loadingLayout">
        ...
        <el-form :model="goods" label-width="110px" style="margin:20px;width:90%;min-width:700px;">
            <el-form-item label="选择文章：">
                <div class="related-goods">
                    <div class="plus-icon">
                        <i class="el-icon-plus" @click="onAddArticle" size="21"></i>
                    </div>
                    <div class="related-goods-item" v-show="(selectArticle.article_id > 0)">
                        <div>
                            <i @click="onRemoveArticle(selectArticle.article_id)" class="el-icon-circle-close"></i>
                            {{selectArticle.title}}
                        </div>
                    </div>
                </div>
        ...
        <el-row class="last">
            <el-col :span="24" class="last-hd">
                <el-radio-group v-model="goods.p_on_sell">
        ...
        <!--关联商品文章弹窗-->
        <el-dialog size="large" v-model="selectArticleLayout" v-loading="loadArticle">
            <div class="search-box">
                <el-input class="input" placeholder="输入你要搜索的文章" icon="search" :on-icon-click="onSearchArticle" v-model="searchKey"></el-input>
            </div>

            <el-table ref="relative-article" @current-change="onArticleChange" :data="articleList" highlight-current-row style="width: 100%;">
                <el-table-column type="index" width="80"></el-table-column>
                <el-table-column prop="title" label="文章名称" min-width="120">
                </el-table-column>
            </el-table>
            <div class="btn-box">
                <el-button type="primary" @click="onConfirmArticle()">确定添加</el-button>
            </div>
        </el-dialog>
    </section>
</template>
	...
	export default {
        ...
        data() {
            return {
                ...
                goods_new_sn: null,
                // 商品详情图片上传
                editUploadImgUrl: config.apiDomain + '/pat/web/goods/upload_desc_img',
                uploadParams: {
        		...
        		goods: {
					...
					relate_article_id: '', // 关联文章ID，非必要
					g_article_title: '', // 关联文章，非必要''
					// sns_attr: '',
					// sns_val: '',
                    p_type: 1,
                    p_on_sell: 1
                ...
                selectArticleLayout: false, // 是否显示搜索推荐文章弹窗
                loadArticle: false, // 搜索文章弹窗
                articleList: [], // 文章列表
                selectArticle: [], // 已选择文章
                tempSelectArticle: [] // 选中文章
            }
        },
        methods: {
            ...
            onAddAttrList() {
                ...
                this.goodsAllAttrs = this.goodsAllAttrs || [];
            ...
            onOkAddGoods() {
                ...
                // 关联文章
                this.goods.relate_article_id = this.selectArticle.article_id;
                console.log(this.goods.relate_article_id);
            ...
            // 选中文章
            onArticleChange(val) {
                this.tempSelectArticle =val;
            },
            // 选择文章
            onConfirmArticle() {
                this.selectArticleLayout = false;
                this.selectArticle = this.tempSelectArticle;
            ...
            uploadImages(img) {
                ...
                return new Promise((resolve, reject) => {
                    ...
                    uploadAdGoodsImg(formdata).then(res => {
                        resolve(res);
                    },
                    err => {
            ...
            async releaseGoods(params) {
                ...
                const _opeGoods = this.goods_sn ? editAdGoods : adAddGoods;
            ...
            // 获取商品详情信息
            getGoodsInfoMsg() {
                getAdGoodsInfo({g_id: this.goods_sn}).then(res => {
                    if (!this.alertBox(res)) return;
            ...
            assignGoodsInfo(data) {
                ...
                this.goods = {
                    ...
                    p_cat: data.cat_id,
                    p_type: data.type,
                    p_on_sell: data.selle_status,
                    relate_article_id: data.g_article_id, // 关联文章id，非必要
                    g_article_title: data.g_article_title // 关联文章，非必要
					// sns_attr: data.name,
					// sns_val: data.name,
                }
                // 遍历标签
                ...
                this.selectArticle.article_id = data.g_article_id;
                this.selectArticle.title = data.g_article_title;
                // 判断是否已经加载了
                this.goodsAllAttrs = this.goodsAllAttrs || [];
                if (this.goodsAllAttrs.length > 0) {
                    // 初始化属性
